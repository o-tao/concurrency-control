...
@Slf4j
@SpringBootTest
class CouponLettuceServiceTest {

    @Autowired
    private CouponLettuceService couponLettuceService;
    @Autowired
    private CouponRepository couponRepository;

    @Test
    @DisplayName("Lettuce기반 RedisLock을 사용해 쿠폰 감소 작업이 동시성 문제 없이 완료된다.")
    public void decreaseWithLettuceLockTest() throws InterruptedException {
        // given
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.execute(() -> {
                try {
                    couponLettuceService.decreaseWithLettuceLock(coupon.getId());
                } finally {
                    latch.countDown();
                }
            });
